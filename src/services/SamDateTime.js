...
var SamDateTime = {
    ...
    addInterval: function (seconds_to_add, interval_type = 'second', received = null) {
        seconds_to_add *= 1000;
    ...
    formattedTime: function (given_time, format) {
        let dt_arr = [];
        let hours = 0;
        let is_not_date = given_time && typeof given_time === 'object' && ('' + new Date(given_time)) == 'Invalid Date';
    ...
    formatMoment: function (given_time, format = 'YYYY-MM-DD') {
        if (!given_time) given_time = Date.now();
        let res = moment(given_time).format(format);
        return res;
    },
    formattedDateTime: function (given_time, format) {
        ...
        let res = SamDateTime.formattedDate(given_time, format) + ' ' + obtained_time;
        return res;
    },
};
