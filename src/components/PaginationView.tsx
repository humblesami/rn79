...
export default class PaginationView extends React.Component<PagingProps, PagingState> {
    ...
    updateDetails() {
        ...
        this.description = (this.offset + 1) + ' to ' + Math.min(this.page_number * on_page, this.total_records);
        ...
        return (
            <View style={{paddingBottom: 10, paddingTop: 10}}>
                <FlexView style={{ padding: 5, justifyContent: 'center' }}>
                        ...
                        <FlexView style={{ justifyContent: 'center' }}>
                            <View style={{}}>
                                <TextInput style={styles.input}
                                    value={this.state.page_str} keyboardType='numeric'
                                    onChangeText={(txt: string) => { obj_it.changePageNumber(txt) }} />
                            </View>
                    ...
                    <View style={[styles.item, {}]}>
                        <TextInput style={styles.input}
                            value={this.state.limit_str} keyboardType='numeric'
                            onChangeText={(txt: string) => { obj_it.changeLimit(txt) }} />
                    </View>                
                </FlexView>
                <View>
                    <Text style={{textAlign: 'center'}}>{this.description}</Text>
