...
export default class AbstractScreen extends Component<PropsType, BaseState> {
    ...
    set_state(args: Record<string, any>, from_point = ''): void {
        ...
        console.log(4444, 'parent set state')
        this.setState((prevState: BaseState) => ({
            ...prevState,
            ...obj_it.temp_state
        }), function () {
            obj_it.temp_state = {};
    ...
    _render_messages(lines: Record<string, string>, heading: string, font_color: string): ReactNode {
        let obj_it = this;
        let error_keys = Object.keys(lines);
        if (!error_keys.length) return null;
        return (
            <ScrollView style={[{ marginTop: 3, maxHeight: windowHeight - 100, paddingBottom: 0 }]}>
                ...
                <View>
                    {error_keys.map((item, i) => {
                        return (
                            <View key={i} style={{ marginBottom: 5, backgroundColor: font_color }}>
                                <Text style={[{ fontSize: 14, padding: 7, borderTopWidth: 1, borderColor: '#eee', color: 'white' }]}>
                    ...
                    <Button onPress={() => { 
                        this.messages = {}; this.issues = {}; obj_it.set_state({}, 'message hidden')
                    }} title='OK' />
                </View>
            </ScrollView>
        );
