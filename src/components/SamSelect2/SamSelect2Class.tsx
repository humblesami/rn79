...
import { Text, View, Alert, Pressable, TextInput, ScrollView, ViewStyle, TextStyle } from 'react-native';
import { Select2Styles } from './Common';
...
class SamSelect2Class extends Component<SamSelect2Props, Select2State> {
    ...
    setTheState(args = {}, from_point = '', calllback=()=>{}) {        
        ...
        if(updated_keys.length == 1 && updated_keys[0] == 'must_do') state_updates = {};
    ...
    onItemRemoved(removedItem: any, index: number) {
        const temp_items = this.state.selectedItems.filter((x) => x.id !== removedItem.id);
        ...
        if (!found) {            
            this.researchItems({selectedItems: temp_items, defaultItems: [...this.state.defaultItems, removedItem]}, 'Item removed');
        }        
        else{
    ...
    async onNewItem(newTag: string) {
        ...
        if (this.attributes.events.onItemAdded) {
            created_item = await this.attributes.events.onItemAdded(created_item, updated_selection);
        }
        let args = {selectedItems: [...this.state.selectedItems, created_item], defaultItems: [...this.state.defaultItems, created_item]};
    ...
    async researchItems(temp_updates={}, from_point = '') {        
        ...
        let filterd_options = this.map_items(this.state.defaultItems, display_field);
        ...
        if(this.state.searchedItems != filterd_options) local_updates.searchedItems = filterd_options;
        if(this.state.selectedItems != filterd_options) local_updates.searchedItems = filterd_options;
    ...
    async onInputFocused() {
        if (!this.state.input_focused) {            
            let res = await this.searchInList(this.state.searchText, {input_focused: true});
    ...
    async searchInList(input_kw: string, received_updates={}) {
        ...
        if (!input_kw) {
            ...
            if (this.attributes.events.onSearch) {
                ...
                let display_field = this.attributes.displayField;
                filtered_items = this.state.defaultItems.filter(x => ('' + x[display_field]).toLowerCase().indexOf(input_kw.toLowerCase()) > -1);
            }
        }
        let tempNotMatched = '';
        ...
        let local_updates = { searchText: input_kw, newTag: tempNotMatched, defaultItems: this.state.defaultItems };
        if (!input_kw && !this.state.defaultItems.length) {
            local_updates.defaultItems = filtered_items;
        }
        return { ...received_updates, searchedItems: filtered_items,  ...local_updates};
    }
    ...
    render_selected_items() {
        ...
        let selected_istyle = Object.assign({}, Select2Styles.selectedItemStyle);
        ...
        if (!this.attributes.multiple) {
            ...
            return (
                <Pressable style={[Select2Styles.selectedItemStyle, Select2Styles.singleSelectedItemStyle]}
                    onPress={() => obj_it.signleSelectionPressed(selected_lis)}>
                    <Text style={{ color: '#555' }}>{selected_lis[0][this.attributes.displayField]}</Text>
        ...
        return (
            <View style={[Select2Styles.selection_container]}>
                {
                    selected_lis.map((toRender, index) => {
                        return (
                            <View key={index} style={selected_istyle}>
                                <Text style={{ color: '#fff' }}>{toRender[this.attributes.displayField]}</Text>
                                <Pressable style={del_btn_style} onPress={() => obj_it.onItemRemoved(toRender, index)}>
                                    <Text style={{ color: 'white' }}>X</Text>
                                </Pressable>
                            </View>
                        )
                ...
                }
                {
                    !this.attributes.prevent_create && this.state.newTag ?
                        <Pressable style={Select2Styles.addBtn}
                            onPress={() => {
        ...
        try {
            ...
            return (
                <View style={{ position: 'relative' }}>
                    <Pressable style={Select2Styles.cancelBtn} onPress={() => obj_it.hideListItems()}>
                        <Text style={{ color: 'white', fontWeight: 'bold' }}>Hide</Text>
                    </Pressable>
                    <ScrollView nestedScrollEnabled={true} style={{ maxHeight: 150, }}>
                        {
                            filteredOptions.map((listItem, index) => (
                                <Pressable key={index} onPress={() => { obj_it.onItemChosen(listItem) }} style={[Select2Styles.stlistItem]}>
                                    <Text>{listItem[this.attributes.displayField]}</Text>
                                </Pressable>
                            ))
                        }
                    </ScrollView>
            ...
            return (
                <ScrollView style={{ maxHeight: 200 }} contentContainerStyle={{ paddingVertical: 10 }}>
    ...
    renderTextInput() {
        ...
        const oldSupport = [
            ...
            { key: 'onChangeText', val: (txt: string) => obj_it.onInputTextChange(txt) },
    ...
    render() {
        return (
            <View style={this.attributes.styling.container}>
                ...
                {(this.attributes.multiple || !this.state.selectedItems.length) ? this.renderFlatList() : <></>}
            </View>
        );
    }
}
